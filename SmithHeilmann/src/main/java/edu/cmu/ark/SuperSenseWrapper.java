...
public class SuperSenseWrapper {
	private SuperSenseWrapper(){
		...
		DiscriminativeTagger.loadProperties(ClassLoader.getSystemResource(GlobalProperties.getProperties().getProperty("propertiesFilePath", "config"+File.separator+"QuestionTransducer.properties")).getFile());
	...
	public List<String> annotateSentenceWithSupersenses(Tree sentence) {
		...
		if(result.size() == 0){
			try {
				if(sst == null){
					sst = DiscriminativeTagger.loadModel(ClassLoader.getSystemResource(GlobalProperties.getProperties().getProperty("supersenseModelFile", "config"+File.separator+"supersenseModelAllSemcor.ser.gz")).toExternalForm());
				}
